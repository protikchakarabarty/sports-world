...
const items: NavItem[] = [
  { key: 'live-matches', label: 'Live Matches' },
  { key: 'latest-news', label: 'Latest News' },
  { key: 'highlights', label: 'Highlights' },
  { key: 'rankings', label: 'Athlete Rankings' },
  { key: 'on-this-day', label: 'On This Day' },
  { key: 'medal-table', label: 'Medal Table' },
  { key: 'hall-of-fame', label: 'Hall of Fame' },
  { key: 'global-records', label: 'Global Records' },
];
...
export function HomeSectionNav() {
  ...
  useEffect(() => {
      ...
      (entries) => {
        for (const entry of entries) {
          if (entry.isIntersecting) {
            setActive(entry.target.id.replace('home-', ''));
          }
        }
      },
    ...
    for (const item of items) {
      const el = document.getElementById(`home-${item.key}`);
