...
import { useWomenFootballFixtures, useWomenPredictions } from '@/hooks/useWomenFootballData';
import { generateAllWomenMatchPredictions } from '@/services/aiPredictionService';
import { getTable } from '@/services/footballApi';
import { detectWomenLeagueCode } from '@/services/providers/sportmonksProvider';
...
export function WomenPredictions() {
  const { t } = useLanguage();
  const { data: fixtures, isLoading: fbLoading } = useWomenFootballFixtures();
  ...
  const { data: predictions, isLoading: predLoading } = useQuery({
    ...
    retry: 1,
    queryKey: ['women-next24h-predictions', ...next24h.map((m) => m.id)],
    queryFn: async () => {
      ...
      await Promise.all(uniqueComps.map(async (comp) => {
        const mapped = detectWomenLeagueCode(comp);
        if (!mapped) return;
        try {
      ...
      return generateAllWomenMatchPredictions(inputs, teamContexts);
    },
