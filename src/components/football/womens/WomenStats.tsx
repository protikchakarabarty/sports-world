...
import { useWomenStats } from '@/hooks/useWomenFootballData';
import { Skeleton } from '@/components/ui/Skeleton';

const LEAGUES = [
  { code: 'WSL', name: 'WSL' },
  { code: 'NWSL', name: 'NWSL' },
  { code: 'LIGAF', name: 'Liga F' },
  { code: 'FBL', name: 'Frauen Bundesliga' },
];

export function WomenStats() {
  const { t } = useLanguage();
  const [showGoals, setShowGoals] = useState(true);
  const [selectedLeague, setSelectedLeague] = useState('WSL');
  const { data, isLoading, isError, error } = useWomenStats(selectedLeague);

  const stats = data ?? [];
  const filtered = showGoals
    ? stats.filter((s) => s.goals > 0).sort((a, b) => b.goals - a.goals).slice(0, 20)
    : stats.filter((s) => (s.assists ?? 0) > 0).sort((a, b) => (b.assists ?? 0) - (a.assists ?? 0)).slice(0, 20);

  return (
    <Section title={t("Women's Football Statistics", 'মহিলা ফুটবল পরিসংখ্যান')}>
