...
import { useWomenTransferRumours, useWomenTransferNews, useWomenDiscussedPlayers } from '@/hooks/useWomenFootballData';
import { Skeleton } from '@/components/ui/Skeleton';
import { MatchCarousel } from '@/components/football/MatchCarousel';
import { timeAgo } from '@/services/womensTransferService';
import type { WomenTransferRumour as WTR } from '@/services/womensTransferService';
...
function RumourCard({ rumour }: { rumour: WTR }) {
  const { t } = useLanguage();
...
function DiscussedCard({ rumour }: { rumour: WTR }) {
  const { t } = useLanguage();
  const ago = timeAgo(rumour.date);
  const countryName = rumour.countryFlag ? getCountryName(rumour.countryFlag) : '';
  const [imgFailed, setImgFailed] = useState(false);
  ...
  return (
    <motion.article
      ...
      className="w-full sm:w-[280px] lg:w-[340px] shrink-0"
    >
      <div className="glass-card rounded-xl overflow-hidden hover:shadow-lg hover:shadow-violet-500/10 transition-all duration-300 hover:-translate-y-1 flex flex-col">
        {/* Player thumbnail */}
        <div className="relative w-full h-48 bg-gray-100 dark:bg-gray-800 shrink-0">
          {showImg ? (
            <img src={rumour.playerImage} alt={rumour.player}
        ...
        <div className="p-3.5 flex flex-col flex-1 gap-2">
          ...
          <div className="flex flex-wrap gap-1 mt-auto pt-2 border-t border-gray-100 dark:border-gray-800 shrink-0">
            ...
            {rumour.sources.slice(0, 2).map((s, i) => (
              <span key={i} className="text-[10px] px-1.5 py-0.5 rounded bg-violet-500/10 text-violet-600 dark:text-violet-400">
                {s}
              </span>
            ))}
          </div>
...
export function WomenTransferMarket() {
  const { t } = useLanguage();
  const { data: rumours, isLoading: rmLoading } = useWomenTransferRumours();
  const { data: news, isLoading: nwLoading } = useWomenTransferNews();
  const { data: discussed, isLoading: dsLoading } = useWomenDiscussedPlayers();

  const [showAllRumours, setShowAllRumours] = useState(false);
  const [showAllNews, setShowAllNews] = useState(false);
  const [showAllDiscussed, setShowAllDiscussed] = useState(false);

  const loading = rmLoading || nwLoading || dsLoading;
  ...
  const hasAnyData = showRumours || showNews || showDiscussed;
  ...
  if (loading && !hasAnyData) {
    return (
      <Section title={t("Women's Transfer Market", 'মহিলা ট্রান্সফার মার্কেট')} subtitle={t('Latest women\'s player transfers and rumours', 'সর্বশেষ মহিলা খেলোয়াড় স্থানান্তর এবং গুজব')}>
  ...
  if (!hasAnyData && !loading) {
    return (
      <Section title={t("Women's Transfer Market", 'মহিলা ট্রান্সফার মার্কেট')} subtitle={t('Latest women\'s player transfers', 'সর্বশেষ মহিলা খেলোয়াড় স্থানান্তর')}>
  ...
  return (
    <Section
      title={t("Women's Transfer Market", 'মহিলা ট্রান্সফার মার্কেট')}
      subtitle={t('Latest women\'s player transfers, rumours, and news', 'সর্বশেষ মহিলা খেলোয়াড় স্থানান্তর, গুজব এবং খবর')}
    >
      {showRumours && displayedRumours.length > 0 && (
        <div className="mb-8">
          <h3 className="text-base font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
            <span>🔥</span> {t('Transfer Rumours', 'ট্রান্সফার গুজব')}
          </h3>
          <MatchCarousel>
            {displayedRumours.map((rumour, ri) => (
              <RumourCard key={rumour.id || `w-rumour-${ri}`} rumour={rumour} />
            ))}
          </MatchCarousel>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {displayedNews.map((item, i) => (
              <NewsCard key={item.url || `w-news-${i}`} item={item} />
            ))}
          </div>
        ...
        <div>
          ...
          <MatchCarousel>
            {displayedDiscussed.map((rumour, di) => (
              <DiscussedCard key={rumour.id || `w-discussed-${di}`} rumour={rumour} />
            ))}
          </MatchCarousel>
