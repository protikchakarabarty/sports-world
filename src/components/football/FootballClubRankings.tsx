...
import { useClubRankings } from '@/hooks/useFootballData';
...
export function FootballClubRankings() {
  const { t } = useLanguage();
  const [search, setSearch] = useState('');
  const [leagueFilter, setLeagueFilter] = useState<string>('all');
  const [showAll, setShowAll] = useState(false);

  const { data: rankings, isLoading, isError, refetch, isFetching } = useClubRankings();
  ...
  return (
    <Section
      title={t('Club Power Rankings', 'ক্লাব পাওয়ার র‌্যাঙ্কিং')}
      subtitle={t('Live club strength rankings across all leagues — dynamically weighted by competition level.', 'সকল লীগ জুড়ে লাইভ ক্লাব শক্তি র‌্যাঙ্কিং — প্রতিযোগিতার স্তর অনুযায়ী ডায়নামিক ওয়েটিং।')}
      gradient="football"
    >
      {/* Controls */}
        ...
        >
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {search
              ...
              : t('No club rankings available.', 'কোনো ক্লাব র্যাঙ্কিং পাওয়া যায়নি।')}
          </p>
        </motion.div>
      )}
              ...
              <tbody>
                <AnimatePresence>
                  {visible.map((entry, i) => {
                    ...
                    return (
                      <motion.tr
                        key={`${entry.club}|${entry.leagueCode}`}
                        layout
