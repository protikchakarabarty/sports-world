...
import { useFootballHighlights } from '@/hooks/useYouTubeHighlights';
...
export function FootballHighlights() {
  const { t } = useLanguage();
  const { data: videos, isLoading } = useFootballHighlights();

  if (isLoading) {
    return (
      <Section title={t('Match Highlights', 'ম্যাচ হাইলাইটস')} subtitle={t('Best moments from recent football matches', 'সাম্প্রতিক ফুটবল ম্যাচের সেরা মুহূর্ত')} link="#">
  ...
  if (!videos || videos.length === 0) {
    return (
      <Section title={t('Match Highlights', 'ম্যাচ হাইলাইটস')} subtitle={t('Best moments from recent football matches', 'সাম্প্রতিক ফুটবল ম্যাচের সেরা মুহূর্ত')}>
        <div className="flex flex-col items-center justify-center py-8 text-center">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {t('No highlights available.', 'কোনো হাইলাইট পাওয়া যায়নি।')}
          </p>
        </div>
      </Section>
    );
