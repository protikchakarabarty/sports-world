...
import { useFootballNews } from '@/hooks/useFootballData';
...
export function FootballNewsSection({ gender = 'men' }: { gender?: Gender }) {
  const { t } = useLanguage();
  const { data: articles, isLoading, error } = useFootballNews(gender);

  if (isLoading) {
    return (
      <Section title={t('Football News', 'ফুটবল খবর')}>
      ...
      <Section title={t('Football News', 'ফুটবল খবর')}>
        <div className="flex flex-col items-center justify-center py-8 text-center">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {t('No football news available.', 'কোনো ফুটবল খবর পাওয়া যায়নি।')}
          </p>
        </div>
      </Section>
    );
