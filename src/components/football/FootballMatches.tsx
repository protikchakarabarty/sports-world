...
export function FootballMatches({ gender = 'men' }: { gender?: Gender }) {
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState('all');
  const { data: fixtures, isLoading, isError, error } = useFootballFixtures(gender);
  const filtered = !fixtures ? [] : activeTab === 'all' ? fixtures : fixtures.filter((m) => m.status === activeTab);

  if (isLoading) {
    return (
      <Section title={t('Football Matches', 'ফুটবল ম্যাচ')} subtitle={t('Live scores, fixtures and results', 'লাইভ স্কোর, ফিক্সচার ও ফলাফল')}>
    ...
    <Section title={t('Football Matches', 'ফুটবল ম্যাচ')} subtitle={t('Live scores, fixtures and results', 'লাইভ স্কোর, ফিক্সচার ও ফলাফল')}>
      ...
      {filtered.length === 0 ? (
        ...
        <>
          ...
          {/* Completed matches — horizontal carousel */}
