...
import { useCricketSeries } from '@/hooks/useCricketData';
...
export function CricketSeries({ gender = 'men' }: { gender?: Gender }) {
  const { t } = useLanguage();
  const { data: series, isLoading } = useCricketSeries(gender);
  ...
  if (isLoading) {
    return (
      <Section title={t('Running Series', 'চলমান সিরিজ')} subtitle={t('Current and upcoming cricket series worldwide', 'বর্তমান ও আসন্ন ক্রিকেট সিরিজ')}>
    ...
    <Section title={t('Running Series', 'চলমান সিরিজ')} subtitle={t('Current and upcoming cricket series worldwide', 'বর্তমান ও আসন্ন ক্রিকেট সিরিজ')}>
      <div className="relative">
        ...
        <div ref={scrollRef} className="flex gap-4 overflow-x-auto scrollbar-none snap-x snap-mandatory pb-2">
          {series.map((s, i) => {
            const seriesKey = s.id ?? s.name ?? `series-${i}`;
            return (
            <motion.div
              key={seriesKey}
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <Badge variant={s.status === 'ongoing' ? 'live' : 'warning'}>
                  {t(s.status === 'ongoing' ? 'Ongoing' : 'Upcoming', s.status === 'ongoing' ? 'চলমান' : 'আসন্ন')}
                </Badge>
